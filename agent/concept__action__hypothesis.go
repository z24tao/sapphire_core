...
func (h *hypothesis) toString(indent string, recursive, indentFirstLine bool) string {
	if h.evaluate() < 0.1 {
		return ""
	}
	...
	result += fmt.Sprintf(indent+"  matches: condition = %d, change = %d, both = %d\n",
		h.conditionMatch, h.changeMatch, h.bothMatch)
	result += fmt.Sprintf(indent+"  condition: %s\n", h.condition.toString(indent+"  ", recursive, false))
	result += fmt.Sprintf(indent+"  change: %s\n", h.change.toString(indent+"  ", recursive, false))
